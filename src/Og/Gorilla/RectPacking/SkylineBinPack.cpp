...
void SkylineBinPack::Insert(std::vector<RectSize> &rects, std::vector<BPRect> &dst, LevelChoiceHeuristic method)
{
	dst.clear();
...
BPRect SkylineBinPack::Insert(int width, int height, LevelChoiceHeuristic method)
{
...
void SkylineBinPack::AddSkylineLevel(int skylineNodeIndex, const BPRect &rect)
{
...
BPRect SkylineBinPack::InsertBottomLeft(int width, int height)
{
	int bestHeight;
	int bestWidth;
	int bestIndex;
	BPRect newNode = FindPositionForNewNodeBottomLeft(width, height, bestHeight, bestWidth, bestIndex);
	...
	if (bestIndex != -1)
	{
		...
		memset(&newNode, 0, sizeof(BPRect));

	return newNode;
}

BPRect SkylineBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestHeight, int &bestWidth, int &bestIndex) const
{
...
BPRect SkylineBinPack::InsertMinWaste(int width, int height)
{
	int bestHeight;
	int bestWastedArea;
	int bestIndex;
	BPRect newNode = FindPositionForNewNodeMinWaste(width, height, bestHeight, bestWastedArea, bestIndex);
...
BPRect SkylineBinPack::FindPositionForNewNodeMinWaste(int width, int height, int &bestHeight, int &bestWastedArea, int &bestIndex) const
{
	bestHeight = std::numeric_limits<int>::max();
