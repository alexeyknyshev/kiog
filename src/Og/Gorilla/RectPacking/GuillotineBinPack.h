...
public:
	...
	enum GuillotineSplitHeuristic
	{
		...
		SplitMinimizeArea, ///< -MINAS, Try to make_unique a single big rectangle at the expense of making the other small.
		SplitMaximizeArea, ///< -MAXAS, Try to make_unique both remaining rectangles as even-sized as possible.
	...
	BPRect Insert(int width, int height, bool merge, FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod);
	...
	/// @param rectChoice The free rectangle choice heuristic rule to use.
	/// @param splitMethod The free rectangle split heuristic rule to use.
	void Insert(std::vector<RectSize> &rects, std::vector<BPRect> &dst, bool merge, 
...
//	void InsertMaxFitting(std::vector<RectSize> &rects, std::vector<BPRect> &dst, bool merge, 
//		FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod);
	...
	std::vector<BPRect> &GetFreeRectangles() { return freeRectangles; }

	/// Returns the list of packed rectangles. You may alter this vector at will, for example, you can move a BPRect from
	/// this list to the Free Rectangles list to free up space on-the-fly, but notice that this causes fragmentation.
	std::vector<BPRect> &GetUsedRectangles() { return usedRectangles; }
...
private:
	...
	std::vector<BPRect> usedRectangles;
	...
	///		rect was placed.
	/// @return A BPRect structure that represents the placement of the new rect into the best free rectangle.
	BPRect FindPositionForNewNode(int width, int height, FreeRectChoiceHeuristic rectChoice, int *nodeIndex);

	static int ScoreByHeuristic(int width, int height, const BPRect &freeRect, FreeRectChoiceHeuristic rectChoice);
	...
	static int ScoreBestAreaFit(int width, int height, const BPRect &freeRect);
	static int ScoreBestShortSideFit(int width, int height, const BPRect &freeRect);
	static int ScoreBestLongSideFit(int width, int height, const BPRect &freeRect);

	static int ScoreWorstAreaFit(int width, int height, const BPRect &freeRect);
	static int ScoreWorstShortSideFit(int width, int height, const BPRect &freeRect);
	static int ScoreWorstLongSideFit(int width, int height, const BPRect &freeRect);
	...
	void SplitFreeRectByHeuristic(const BPRect &freeRect, const BPRect &placedRect, GuillotineSplitHeuristic method);
